...
export async function getCabin(id) {
  ...
  if (error) {
    console.error(error);
    // this actually triggered the manually written 'not-found.js'
    notFound();
  }
...
export const getCabins = async function () {
  ...
  return data;
  // const res = await fetch(
  //   "https://backend-world-wide-oasis.onrender.com/api/cabins"
  // );
...
export async function getBookings(guestId) {
  const { data, error, count } = await supabase
    ...
    .select(
      "id, created_at, startDate, endDate, numNights, numGuest, totalPrice, guestID, cabinID, cabin(name, image)"
    )
    .eq("guestID", guestId)
    .order("startDate");
